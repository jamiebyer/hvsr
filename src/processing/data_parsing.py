...
def get_station_file_mapping():
    ...
    out_df.loc[:, "GNSS_latitude"] = (
        out_df.loc[:, "GNSS_latitude"].astype(float).round(3)
    )
    out_df.loc[:, "GNSS_longitude"] = (
        out_df.loc[:, "GNSS_longitude"].astype(float).round(3)
    )
    # out_df.loc[:, "GNSS_elevation"] = out_df.loc[:, "GNSS_elevation"].round(3)

    out_df.loc[:, "site"] = (
        out_df.groupby(["GNSS_latitude", "GNSS_longitude"]).ngroup() + 1
    ).values
...
def plot_station_schedule():
    ...
    for ind, recording in df_mapping.iterrows():
        dates = pd.date_range(
            start=recording["Start_time (UTC)"],
            end=recording["End_time (UTC)"],
            inclusive="both",
        )
        plt.plot(
            dates, len(dates) * [recording["site"]], linestyle="solid", color="blue"
        )
